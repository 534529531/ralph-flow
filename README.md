...
开放式任务、Bug 修复、范围明确的功能开发。一个步骤内完成「实现 → 执行摘要 → 挑剔验证 → 修复」的迭代闭环，直到验证者挑不出毛病。验证者独立建立完成标准、对照真实变更、重跑验证命令、对抗找破绽。默认 `reset: true`：每轮失败重试换干净上下文，失败原因随 retryContext 带入。
...
```mermaid
flowchart LR
    L["loop<br/>（实现 → 摘要 → 对抗验证）"] -->|不通过<br/>带失败原因重试| L
    L -->|通过| Done
```
