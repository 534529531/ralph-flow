...
const activeChecks = new Map<string, ActiveCheck[]>(); // instId -> running verifier handles (multi-voter)

/** Abort all still-running verifiers for an instance (in-process only). */
export function abortActiveCheck(client: Client, instId: string): void {
  const handles = activeChecks.get(instId);
  ...
  for (const h of handles) {
    ...
    Promise.resolve()
      .then(() => client.session.abort({ path: { id: h.checkSessionId } }))
      .catch(() => {})
      .then(() => client.session.delete({ path: { id: h.checkSessionId } }))
      .catch(() => {});
  }
}
...
export function isCheckSession(sessionId: string): boolean {
  for (const handles of activeChecks.values()) {
    for (const h of handles) {
      if (h.checkSessionId === sessionId) return true;
    }
  }
  return false;
}
...
export async function adversarialCheck(
  ...
  return runSingleVoter(client, engine, instId, ownerSessionId, step, checkPrompt, userTask, adversarialConfig);
}

export interface VoterOpts {
  /** 1-based voter index in the voting round (session title suffix). */
  index?: number;
  /** Total voter count (session title suffix). */
  count?: number;
}

/**
 * Run ONE independent verifier session. Shared by the single-check path
 * (adversarialCheck) and the voting path (check-voting.ts). The model
 * resolution chain: explicit model → agent config → owner session → global.
 */
export async function runSingleVoter(
  ...
  engine.logEvent(instId, "info", "adversarial_check_start", { stepId: step.id, agent, model: model ? `${model.providerID}/${model.modelID}` : "agent-default", model_source: modelSource, timeout_ms: timeout, extra_dirs: extraDirs, voter_index: voter?.index });
  ...
  engine.logEvent(instId, "info", "adversarial_check_prompt", { stepId: step.id, systemPrompt: truncate(systemPrompt), checkPrompt: truncate(checkPrompt), voter_index: voter?.index });
  ...
  try {
    const checkSession = await client.session.create({
      body: {
        title: `Ralph Check: ${step.id}${voterTag} - ${(userTask || "").substring(0, 50)}`,
    ...
    engine.logEvent(instId, "error", "adversarial_check_session_create_failed", { stepId: step.id, error: err.message, voter_index: voter?.index });
  ...
  const existing = activeChecks.get(instId) ?? [];
  existing.push(handle);
  activeChecks.set(instId, existing);
  ...
  if (!engine.instanceExists(instId)) {
    const arr = activeChecks.get(instId);
    if (arr) {
      const next = arr.filter((h) => h.checkSessionId !== checkSessionId);
      if (next.length > 0) activeChecks.set(instId, next); else activeChecks.delete(instId);
    }
    try { await client.session.delete({ path: { id: checkSessionId } }); } catch {}
    engine.logEvent(instId, "warn", "adversarial_check_cancelled_before_start", { stepId: step.id, voter_index: voter?.index });
    return { passed: false, reason: "工作流实例已被取消。" };
  }
  ...
  const keepalive = setInterval(() => {
    engine.logEvent(instId, "info", "adversarial_check_keepalive", { stepId: step.id, elapsed_ms: Date.now() - startTime, voter_index: voter?.index });
  }, 60_000);
  ...
  try {
    ...
    try {
      ...
      if (String(err.message).includes("timeout")) {
        ...
        engine.logEvent(instId, "warn", "adversarial_check_timeout", { stepId: step.id, voter_index: voter?.index });
        return {
          passed: false,
          infra: true,
...
4. 如果任务确实需要更长时间，可以在工作流配置中增加 timeout_ms`,
      ...
      engine.logEvent(instId, "error", "adversarial_check_error", { stepId: step.id, error: err.message, voter_index: voter?.index });
    ...
    if (requestError) {
      engine.logEvent(instId, "error", "adversarial_check_request_failed", { stepId: step.id, agent, model: model ? `${model.providerID}/${model.modelID}` : "agent-default", error: requestError, voter_index: voter?.index });
      return {
        passed: false,
        infra: true,
...
修复配置（或稍等恢复）后运行 /ralphflow-continue 重新验证。`,
    ...
    engine.logEvent(instId, "info", "adversarial_check_result", { stepId: step.id, passed, len: responseText.length, reason: reason.substring(0, 160), voter_index: voter?.index });
    ...
    engine.removeAdversarialSession(checkSessionId, instId);
    try { await client.session.delete({ path: { id: checkSessionId } }); } catch {}
  }
}
