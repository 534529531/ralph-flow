...
**可选的 \`reason\` 参数**：
- 如果用户说了**为什么**要重置、重做时希望注意什么，提炼成简短的一段话作为 \`reason\` 参数传入——它会跨会话注入到新会话首条 DO 提示前，并随空闲保活一直保留
- 用户没说就不传：行为与旧版一致
...
- 失败计数**保留**——重置只换干净的上下文，不会重置工作流状态（想重算失败预算，先等工作流暂停再用 \`/ralphflow-continue\` 显式恢复；想回退到**上游已通过步骤**改方向重做，用 \`/ralphflow-rewind\`）

调用 \`ralphflow_reset\`（不带参数对本会话的实例；本会话有多个实例时需指定 \`instance\` 参数，支持唯一前缀）。不要猜测——只在用户确实要求时调用。`,
  },

  "ralphflow-rewind": {
    description: "回退到本工作流里已通过 CHECK 的上游步骤，重做该步及后续",
    template: `调用 \`ralphflow_rewind\` 工具回退到一个**之前已经通过独立 CHECK**的上游步骤，重新执行该步及之后。

用户输入：$ARGUMENTS

**何时用它**（这是**方向回退**场景，和 \`/ralphflow-reset\` 区分）：
- 后期步骤做到了一半或后期做完，回头发现**早期的步骤方向走错了**——比如第二步的技术文档里 API 假设错了，第四步才暴露，于是回到第二步重来
- 你已经知道要回到的**具体哪一步**（用 \`/ralphflow-status\` 看现在在哪、用 \`/ralphflow-list\` 看历史通过过哪些步骤）
- 暂停的工作流里也可以走这条路——很多用户正是发现卡住之后才决定回退

**参数**：
- \`step\`（**必填**）：目标步骤 id。必须是本工作流里**已通过独立 CHECK**的上游步骤——不能是当前步骤、未来步骤、或子工作流（复合）步骤
- \`reason\`（**必填**）：回退原因。来自用户，落地为简短一段话：**为什么**回退、**重做时希望注意什么**。它会跨会话注入新会话首条 DO 提示前并随空闲保活保留——这是新会话冷启动时唯一的方向指引，**不能**瞎填（"用户要求回退"这种空话会让模型毫无方向地白重做一遍）
- \`keep_session\`（可选，默认 \`false\`）：是否在当前会话继续而不换新会话。默认换入干净会话（与 \`/ralphflow-reset\` 一致），因为"回退通常因为当前上下文已被前面对话污染";想让模型在你当前眼皮底下重做就传 \`true\`
- \`instance\`：实例 id 前缀，仅本会话有多个实例时需要

**为什么 reason 必填**：这是用户旅程的关键断点——旧版 reset 丢失用户跟你说"为什么回退"的原因，导致新会话冷启动只收到一句"步骤 X 将重新执行"，毫无方向感。rewind 把这条原因跨会话带过去，让模型一开局就知道这次重做要纠正什么。

**执行后会发生什么**：
- 状态机倒退到目标步骤的 DO 阶段；\`fail_count\` **归零**，\`paused\` **清除**
- 下游已落盘的代码/文档/改动**全部保留**——插件不会自动删除任何产物。请用户按 \`reason\` 决定是保留、调整还是让模型覆盖旧产出
- 默认换新会话：当前会话历史保留可切回查看，工作流驱动权转移到新会话
- 调用 \`ralphflow_rewind\`，把用户的 \`step\` + \`reason\` 准确传入（可选 \`keep_session\`/\`instance\`）。如果用户没说清 \`step\` 或 \`reason\`，先追问确认，不要猜。

**信息不全时怎么追问**（一轮问清）：
- 用户没说要回到哪一步 → 看 \`/ralphflow-status\` 并把历史已通过步骤列给用户，问"想回到哪一步重做"
- 用户说"回到第二步"但没说为什么 → 问"为什么回退？这次重做希望注意什么？"，拿到简短一段话作为 \`reason\`
- 用户既没要哪步也没说原因 → 两问一起问，别多次来回打断`,
  },
...
- **检查清单模式** 适合开放式任务：第一步把需求拆解成一份 \`summary.md\`，其中每一项都客观可验证并标注验证方法；后续步骤执行并勾选；它们的 \`check\` 独立地重新验证每一项，而不是相信勾选。
