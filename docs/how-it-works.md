...
`on_pass`/`on_fail` 是工作流**作者**预设的静态路由——只有当 CHECK 判定失败时引擎才按图跳。三个常用的运行时场景都不依赖它：

| 场景 | 命令 | 状态机 | 计数 | 换会话 | reason |
|------|------|--------|------|--------|--------|
| 当前步上下文脏了想原地重来 | `/ralphflow-reset` | 不动（停当前步） | **保留**——不赦免，自动刹车仍然真 | 默认换干净会话 | 可选 |
| 已经做到后期，发现前面某步方向错了，要回去重做 | `/ralphflow-rewind` | 倒退到一个**已通过 CHECK**的上游步骤 | 归零 + 清暂停 | 默认换干净会话 | **必填**，跨会话注入新会话首条 DO 提示前 |
| 整个推倒重来 | `/ralphflow-cancel` + `/ralphflow-start` | 销毁实例 | — | — | — |

### 为什么 rewind 的 reason 必填

旧版 `/ralphflow-reset` 把用户跟 AI 说的"为什么重置"丢在旧会话——新会话冷启动只收到一句"步骤 X 将重新执行"，毫无方向感。rewind 的回退场景更吃这条断点：用户回退到第二步重做技术文档，对模型说"API 假设错了"，这才是第二步重做的关键输入。rewind 把它落到 `reason` 参数、跨会话注入新会话首条 DO 提示前，并随 idle keep-alive 重注入一直保留——新会话冷启动第一眼就带着用户给出的方向，几轮对话后也不会被冲掉。

`/ralphflow-reset` 也顺手补了可选的 `reason`：不传时行为与旧版字节级一致；传了就走同一条跨会话注入路径。

### rewind 的边界

- **目标限定为本工作流里已通过独立 CHECK 的步骤**（用 step records 里 `phase==="check" && status==="passed"` 判定）——回退到没通过过的步骤、未来步骤、当前步都被拒绝，且回退到当前步时指向 `/ralphflow-reset`。记录按产生时所在的工作流名隔离：子工作流里通过的同名步骤不会让父工作流的同名步骤变成可回退目标（2.7.1 之前的旧记录无工作流名，回退为仅按步骤 id 匹配）。
- **回环流程里的"回退"语义**：目标只要求"曾通过 CHECK 且非当前步"，不限在工作流数组里的顺序——带回环（`on_fail` 指回上游）的工作流跑几圈后，可回退目标可能在数组顺序上靠后，此时 rewind 实际是跳到那个步骤重做。这是用户显式发起、且必须给 reason 的操作，语义即"跳到任何曾通过的步骤"。
- **不动下游产物**：插件**不会自动删除**任何已落盘代码/文档/改动。回退说明里明确告诉新会话："按 reason 自行决定保留、调整还是让模型覆盖旧产出"。
- **不支持跨子工作流栈帧回退**：实例在子工作流里（state stack 非空）时一律拒绝——回到父工作流步骤需要决定弹栈到哪一层、把哪几层进度作废，第一版留白；更直接的做法是 `/ralphflow-cancel` 后重新 `start`。
- **paused 实例允许 rewind**：暂停时 `current_phase` 通常是 `do`，回退顺便清掉暂停 + 归零 fail_count——和"用户发现卡死之后决定换个方向重来"的旅程对齐。（对比：`/ralphflow-reset` 在暂停时会**拒绝**并指向 continue——paused 会随状态带进新会话，新会话的空闲驱动对暂停实例静默，看似"没反应"。）

设计细节与历史决策（parentID 撤回、abort 顺序、briefing 头部等）见 `docs/reset-gate-design.md`。
