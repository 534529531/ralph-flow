...
`true` 时，**每次跨步骤转换**都换入一个全新的上下文会话（等价于给每个步骤都标 `reset: true`）。缺省 `false`。

```yaml
auto_reset: true
```

适合多步长程工作流（每一步都在干净上下文里发挥最佳质量），代价是每步重建现场的 token 成本。粒度更细的控制用步骤级 `reset: true`（见[上下文重置门](#上下文重置门reset)）。纯线性工作流（每步 `on_fail` 都指回自身）开 `auto_reset` 时 doctor 会给成本提示。
...
| 工作流级 | 顶部 `auto_reset: true` | 多步长程流，每次跨步骤转换都换 |
| 手动 | 随时运行 `/ralphflow-reset` | 感觉当前会话"脏了"，立即换个干净的重做当前步骤 |

### 触发规则

- **只在跨步骤转换时触发**（CHECK 判定后进入另一步骤）。同步骤重试（`on_fail` 指回自身）**不触发**——失败重试恰恰需要现场记忆。
- `auto_reset` 与步骤级 `reset` 是**或**关系。
- 嵌套工作流完整适配：标记可以打在 **`workflow:` 复合步骤上**（进入该子工作流前换会话），也可以打在**子工作流内部**的普通步骤上（子工作流自己就是个工作流，规则相同）。子工作流失败沿 `on_fail` 回炉重进复合步骤时同样判定。
- 手动重置只能在 **DO 阶段**进行（CHECK 阶段有独立验证在跑）；`fail_count` **不赦免**——想清零失败预算走「暂停 → `/ralphflow-continue`」的显式路径。
...
> 首步标 `reset` 时启动不触发（启动时本来就是新会话），但后续步骤失败 `on_fail` 回到首步时仍会触发——此时上下文已膨胀，reset 有意义。
